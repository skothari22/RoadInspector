...
public class Photo {
    ...
    // TODO : does not work well for characters of similar size to target size
    public BufferedImage averageResizeBi(BufferedImage origin, int width, int height) {
        ...
        if (origin.getWidth() < width || origin.getHeight() < height)
            return linearResizeBi(origin,width,height); // average height does not fit
         // to swap, so if we zoom in x or y, we'll use it
         // line of linear transformation

         /* java api standardly reduces images by the bilinear method, respectively. linear mapping.
          * which brings about a lot of loss of information. Ideal would be fourier
          * Transformation, but it does not come to the conclusion of the great cesare factuality
          * therefore, the WEIGHTED AVERAGE method is optimal
          */
        ...
        for (int x=0; x<width; x++) {
            ...
            for (int y=0; y<height; y++) {
                ...
                // fix the average of the environment and save it to resizedImage;
                float sum = 0;
                int sumCount = 0;
    ...
    public void adaptiveThresholding() { // only use of this feature should be in the brand constructor 
